defmodule QuickestRoute.Search.GoogleTest do
  ...
  describe "refine_place/2" do
    test "returns an :ok Place when proper json returned" do
      ...
      assert %Place{status: :ok, original: ^place, refined: ^expected_candidates} = Google.refine_place(place, "abc")
    end
    ...
    test "retuns an :error Place when non-200 json returned" do
      ...
      assert %Place{status: :error, original: ^place, error_message: "Unable to refine place \"user input\" for search"} = Google.refine_place(place, "abc")
    end
  end
end
